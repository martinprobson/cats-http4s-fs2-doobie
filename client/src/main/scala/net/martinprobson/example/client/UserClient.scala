...
import net.martinprobson.example.files.ErrorFileWriter
...
object UserClient extends IOApp.Simple {
  ...
  /** Given a client and a source stream of Users, call the [[postUsers]] method to post them to the server and process
    * the results. <p>Any errors in the result stream are written out to two files, one containing the failed Users (in
    * Json format) and the other containing the error message and user.</p>
    * @param client
  ...
  /** Main entry point for our client program. <ol> <li>Generate some files containing Json encoded
    * [[net.martinprobson.example.common.model.User]]'s</li> <li>Call our program with the file source.</li> </ol>
    */
  ...
  private def postUser(user: User, client: Client[IO]): IO[Either[(String, User), User]] = {
    def req(user: User): Request[IO] = Request[IO](method = Method.POST, uri"http://localhost:9090/user")
      .withEntity(user)
